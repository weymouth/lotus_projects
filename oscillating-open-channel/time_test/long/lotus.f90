...
program channel
  ...
  ! - Domain
    ...
    real               :: m(3) = [2*1./8, 2., 2*1./8]
  ! - Length scales
    integer,parameter  :: S = 128  ! Stokes length
  ...
  ! -- Initialize MPI
    ...
    xg(1)%h = 8.0
    call xg(2)%stretch(n(2),0.,0.,1.*S,4.0*S,prnt=root)
    xg(3)%h = 8.0
  !
  ! -- Init channel
  ...
  !
    ...
    ! - Write some stuff
      ...
      if(root) write(9,'(f10.4,f8.4,4e16.8)') t/(2*pi*S),dt/t/(2*pi*S), &
              enstrophy, tke
    end do
    !
  ...
  real(8) pure function position(t)
    real(8),intent(in) :: t
    position = 690/0.12*S*sin(t/S)
  end function position
  real(8) pure function velocity(t)
    real(8),intent(in) :: t
    velocity = 0.12*cos(t/S)
