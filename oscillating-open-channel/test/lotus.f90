...
program channel
  ...
  ! - Domain
    ...
    real               :: m(3) = [0.5*1./8, 0.5, 0.5*1./8]
  ! - Length scales
    integer,parameter  :: a = 256
    real,parameter     :: nu = a/102000
    real               :: h_roughness=0.05*a,thickness,lambda=a*1./4
  ! - Time scales
    real,parameter     :: St=0.318, f=St/(2.*a)
    real,parameter     :: finish=2., print_int=0.05, init_time=finish-1.
    real               :: t,dt
  ! - Init sim
    type(fluid)        :: flow
    type(body)         :: wall
  ...
  ! -- Initialize MPI
    ...
    xg(1)%h = 8.0
    call xg(2)%stretch(n(2),0.,0.,0.25*a,2.0*a,prnt=root)
    xg(3)%h = 8.0
  !
  ! -- Init channel
