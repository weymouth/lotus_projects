...
try:
    vof = pd.read_csv('fort.7',delim_whitespace = True,
        names=["loops","flow","vof","limit"])
except FileNotFoundError:
    exit('stat: fort.7 not found')
#
# -- plot PDF pages
...
with PdfPages('history.pdf') as pdf:
    ...
    vof.plot(y=['flow','vof','limit'],figsize=(8,4))
    plt.ylabel('dt')
    pdf.savefig()

    vof.plot(y='loops',figsize=(8,4))
    pdf.savefig()
