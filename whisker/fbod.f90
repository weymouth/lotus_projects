...
contains
  subroutine init_body(del,ub)    
    ...
    if(eps.eq.0.D0)  then
       call io_error('body_init: must set eps')
    else if(eps.lt.0.D0) then
       eps = grid_size()*sqrt(-eps)
    end if
...
!
! -- get switch function and velocity on cell faces
    do d=1,ndims
       o = 0; o(d) = 1
       do i=is,ie+o(1)
       do j=js,je+o(2)
    ...
    call mympi_vector(del)
    call mympi_vector(ub)
!
! -- get distance and force kernel on cell centers
    do i=1,ni
    do j=1,nj
    do k=1,nk 
