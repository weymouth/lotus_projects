...
module nurbs
  ...
  type pvec
     real(8) :: uv(2),pos(3),dpdu(3),dpdv(3),norm(3)
  end type pvec
  type bspline
     integer             :: n,order
...
!-------------------------------------------------------!
!
! -- Get the normal at every point on the surface
!
! note:  poles do not have both u & v tangents but can 
! note:  still have a well defined normal direction
!
  subroutine surf_norm(srf)
    implicit none
    type(trim),intent(inout),target :: srf
    real(8),pointer,dimension(:) :: n,u,v
    integer :: i,j
    real(8) :: a
    do i=1,np
    do j=1,np
       ...
       u => srf%pv(i,j)%dpdu
       v => srf%pv(i,j)%dpdv
       if(sum(u*u)<1e-9) then       ! |u|=0; try using v
          u => srf%pv(np-1,j)%dpdv
          v => srf%pv(   2,j)%dpdv
       else if(sum(v*v)<1e-9) then  ! |v|=0; try using u
          u => srf%pv(i,2   )%dpdu
          v => srf%pv(i,np-1)%dpdu
       end if
       n(1) = u(2)*v(3)-u(3)*v(2)
       n(2) = u(3)*v(1)-u(1)*v(3)
       n(3) = u(1)*v(2)-u(2)*v(1)
       a = sqrt(sum(n*n)) 
       if(a<1e-9) stop 'undefined normal'
       n = n/a
    end do
    end do
