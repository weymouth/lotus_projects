...
!
  ...
  interface io_log_field
   ...
   integer              :: force_num = 10, press_num = 8, CFL_num = 11 !and 12
   integer              :: motion_num = 13 
...
!
  subroutine io_profile_report(t)
    ...
    if(t.eq.tmx) then                        ! say goodbye
       ...
       write(io_log,2) t,hours,minutes,seconds
    end if
    call log_print
    ...
    sort_loop: do i=1,profile_num
       ...
       unsorted(j) = .false.
       if(profile_dt(j).lt.0.005*time.or.profile_dt(j).lt.1) exit sort_loop
       write(io_log,1) i,profile_name(j),100*profile_dt(j)/time,profile_dt(j)
       call log_print
    end do sort_loop
    call mympi_report()
...
2   format('Time step =',i5,', ETR: ',i3,'hrs, ',i2,'mns, ',i2,'scs')
