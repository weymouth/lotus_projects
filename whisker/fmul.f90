...
!-------------------------------------------------------------
  subroutine init_solver
    ...
    call init_PCG(set_it_mx=500,set_tol=tol,debug=.true.)
    call init_MG(set_it_mx=mx,set_tol=tol,set_smoother='PCG  ',&
         set_prolongate='linea',set_adjust=.false.,debug=.true.)
...
! ---------------------------------------------------------
!
  subroutine solver_invert(time,x,b)
    ...
    if(time.eq.0.D0) call PCG_solver(A,x,b)  ! help initialize the pressure
