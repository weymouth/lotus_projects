...
!------------------------------------------------------------!
module mympi
  integer,private,allocatable,dimension(:) :: coords,me,sub_points,blocks
  integer,private                          :: myid,myidmx,ios,mpi_grid
  integer,private                          :: gis,gie,gjs,gje,gks,gke
  real(8),private                          :: mpi_time
  logical,private,allocatable,dimension(:) :: periodic
contains
!----------------------------------------------------------------
!
...
! -- Read mpi inputs
!
    allocate(blocks(0:ndims-1),periodic(0:ndims-1),points(ndims))
    call mympi_read(input_num)
    call mympi_read(input_num,la=periodic)
    call mympi_read(input_num,ia=blocks)
!
! -- but ignore blocks since this is serial
!
    blocks = 1
    myidmx = 0
