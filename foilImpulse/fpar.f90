...
module mympi
  ...
  include 'mpif.h'
  integer,private,allocatable,dimension(:) :: coords,points,me,blocks
  ...
  logical,private,allocatable,dimension(:) :: global_subflag
...
!
    ni = points(0)/blocks(0)+2*thk; is = 1+thk; ie = ni-thk
    nj = points(1)/blocks(1)+2*thk; js = 1+thk; je = nj-thk
    if(ndims.eq.3) then
       nk = points(2)/blocks(2)+2*thk; ks = 1+thk; ke = nk-thk
