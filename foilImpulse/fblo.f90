...
!
    xRoot = Find(S,x)               
    yRoot = Find(S,y)              
    if(S(xRoot)%rank.gt.S(yRoot)%rank) then 
       S(yRoot)%parent = xRoot
    elseif(S(xRoot)%rank.lt.S(yRoot)%rank) then 
       S(xRoot)%parent = yRoot
    elseif(xRoot.ne.yRoot) then 
       S(yRoot)%parent = xRoot
...
!
  subroutine blob_find(r,cutoff,d_num,h_num,time,iout)
    real(8),intent(inout)        :: r(:,:,:)
    integer,intent(in),optional  :: cutoff,d_num,h_num
    ...
    type(data) :: b
    integer    :: ni,nj,nk,n,i,j,k,d,p(3),p2(3),label,local
!
! -- init
       ...
       do d=3,1,-1
          ...
          if(local.ne.0) then            ! local tree exists
             if(label.ne.0) &               ! labeled tree exists
                call Union(blob,local,label)   ! merge trees
             label = local                  ! label tree
          end if
       end do
!
