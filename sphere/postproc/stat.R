...
adrag = sqrt(2*sd(late$drag))
mlift = mean(late$lift)
alift = sqrt(2*sd(late$lift))
drag = qplot(time,drag,data=data,geom="line")
drag = drag+annotate("text",x=t,y=mdrag+0.1,label=paste("mean=",round(mdrag,3)," amp=",round(adrag,3)))
lift = qplot(time,lift,data=data,geom="line")
lift = lift+annotate("text",x=t,y=mlift+0.1,label=paste("mean=",round(mlift,3)," amp=",round(alift,3)))
