program enstrophy_test
  ...
  ! real              :: lbox(3)=[-0.25,-0.25,0.],hbox(3)=[0.25,0.25,0.]
  real              :: lbox(3)=[-0.,-0.33,0.],hbox(3)=[1.,1.,0.]
!
! -- Set parameters
    nu = L/(Re)
...
! -- Initialize the velocity field
    ...
    ! call flow%reset_u0()
    ! omega = flow%velocity%tke()
    omegabox = flow%velocity%tke(mean=.true.,lowerb=lbox*L, upperb=hbox*L)

    if(mympi_rank()==0) write(*,*) '--- TKE Test --- '
    if(mympi_rank()==0) write(*,*) 'Enstrophy',omega,omegabox!,flow%velocity%tke(lowerb=lbox*L, upperb=hbox*L),flow%velocity%tke()
    call mympi_end()
  contains
