program swimming_plate
  ...
  !
  ! -- Physical parameters
    real,parameter     :: Re = 1000
  !
    real,parameter     :: L=256, nu=L/Re
    real, parameter    :: finish=0.1, lbox(3)=[0.,-0.2,0.], ubox(3)=[1.2,0.2,0.03125*4]
    integer            :: b(3) = [2,2,1]
  !
  ! -- Hyperparameters
    real, parameter    :: thicc=0.03*L
    real, parameter    :: A = 1.*L, St_d = 0.3, k_x=0., k_z=32.0, h_roughness=0.00
    real, parameter    :: a_coeff = 0.28, &
                          b_coeff = -0.13, &
                          c_coeff = 0.05, &
                          k_coeff = 0.8695652173913044, &
                          f = St_d/(2.*A)
  !
  ! -- Dimensions
    integer            :: n(3), ndims=2
  ...
  ! -- Setup solver
    ...
    !
    ! -- Outputs
      real            :: dt, t, pforce(3), ppower, enstrophy
    !
    ! -- Initialize
    ...
    ! -- Setup the grid
      ...
      call xg(1)%stretch(n(1), -1.5*L, -.5*L, 2.5*L, 5.0*L,  h_min=4., h_max=10., prnt=root)
      call xg(2)%stretch(n(2), -1.25*L, -0.8*L, 0.8*L, 1.25*L, h_min=2., prnt=root)
      if(ndims==3) xg(3)%h = 4.
    !
    ...
    !
      time_loop: do while (flow%time<finish/f.and..not.there)
        ...
        write(9,'(f10.4,f8.4,4e16.8,4e16.8,4e16.8,4e16.8,4e16.8,4e16.8,&
              ...
              t*f,dt,pforce,ppower,enstrophy
        ...
        enstrophy = flow%velocity%enstrophy(.true.)!, L*lbox,L*ubox)!/(n(1)*n(2)*n(3))
  
        if((mod(t,1./f)<dt).and.(root)) print "('Time:',f15.3)",&
        t*f
        ! if((t>(finish-2)/f).and.(mod(t,0.1/f)<dt)) call flow%write(geom, write_vtr=.false.)
      end do time_loop
